...
class BaseTrainer:
            ...
            if mode == 'train':
                ...
                if is_acc_step or is_final_step:
                    if self._cfg.max_grad_norm > 0:
                        for i in range(len(self._model)):
                            self._model[i].clip_grad(self._cfg.max_grad_norm)
                    for i in range(len(self._model)):
                        self._model[i].update()
                        self._model[i].zero_grad()
    ...
    def _save_ckpt(self):
        for i in range(len(self._model)):
            self._model[i].save_state(self._epoch, self._stat.stat, self._ckpt_dir)
