...
class ELMo(nn.Module):
    def __init__(self, num_embeddings, padding_idx, vocab_size):
        ...
        self.charCNN = CharEmbedding(num_embeddings, embedding_dim, padding_idx, conv_filters, 2, projection_size)
        self.m_forward_lstm_1 = torch.nn.LSTM(512, 2048, 1)
        self.m_forward_fc_1 = torch.nn.Linear(2048, 512)
        self.m_forward_lstm_2 = torch.nn.LSTM(512, 2048, 1)
        self.m_forward_fc_2 = torch.nn.Linear(2048, 512)
        self.m_backward_lstm_1 = torch.nn.LSTM(512, 2048, 1)
        self.m_backward_fc_1 = torch.nn.Linear(2048, 512)
        self.m_backward_lstm_2 = torch.nn.LSTM(512, 2048, 1)
        self.m_backward_fc_2 = torch.nn.Linear(2048, 512)
        self.out = torch.nn.AdaptiveLogSoftmaxWithLoss(in_features=512, n_classes=vocab_size, cutoffs=[10, 100, 1000])

    def forward(self, forward_X, forward_Y, backward_X, backward_Y):
        forward = self.charCNN(forward_X).transpose(1, 0)
        forward, (_, _) = self.m_forward_lstm_1(forward)
        forward = self.m_forward_fc_1(forward)
        forward_of1_feat = forward
        forward, (_, _) = self.m_forward_lstm_2(forward)
        forward = self.m_forward_fc_2(forward) + forward_of1_feat
        forward = forward.transpose(1, 0).contiguous()
        forward = forward.view(-1, forward.size(2))
        forward_Y = forward_Y.view(forward_Y.size(0)*forward_Y.size(1))
        forward = self.out(forward, forward_Y)
        forward_out = forward.output.view(forward_X.size(0), forward_X.size(1))

        backward = self.charCNN(backward_X).transpose(1, 0)
        backward, (_, _) = self.m_backward_lstm_1(backward)
        backward = self.m_backward_fc_1(backward)
        backward_of1_feat = backward
        backward, (_, _) = self.m_backward_lstm_2(backward)
        backward = self.m_backward_fc_2(backward) + backward_of1_feat
        backward = backward.transpose(1, 0).contiguous()
        backward = backward.view(-1, backward.size(2))
        backward_Y = backward_Y.view(backward_Y.size(0)*backward_Y.size(1))
        backward = self.out(backward, backward_Y)
        backward_out = backward.output.view(backward_X.size(0), backward_X.size(1))
